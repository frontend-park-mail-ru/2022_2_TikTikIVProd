...
import signupConfig from './SignupFormViewConfig.js';
...
export default class SigninView extends IComponent {
    ...
    render() {
        ...
        const formHeader = createDiv({
            text: signupConfig.title,
            styles: ['form__title'],
        });

        const formContent = createDiv({});
        signupConfig.fields.forEach((field) => {
        ...
        submitBtn.appendChild(
            createButton({
                id: signupConfig.submit.id,
                text: signupConfig.submit.text,
        ...
        signupConfig.footer.forEach((link) => {
